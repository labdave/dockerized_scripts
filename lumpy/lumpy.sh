...
# copy file
cp "${bam}" /usr/local/bin/sample.bam
cd /usr/local/bin/
...
samtools sort /usr/local/bin/sample.discordant.bam > /usr/local/bin/sample.discordant.sorted.bam
samtools index /usr/local/bin/sample.discordant.sorted.bam
echo "discordant reads sorted" 1>&2

samtools sort /usr/local/bin/sample.split.bam > /usr/local/bin/sample.split.sorted.bam
samtools index /usr/local/bin/sample.split.sorted.bam
...
lumpy -mw 4 -tt 0 \
-pe id:sample,bam_file:/usr/local/bin/sample.discordant.sorted.bam,histo_file:/usr/local/bin/sample.lib.histo,mean:$mean,stdev:$std,read_length:"${read_length}",min_non_overlap:"${read_length}",discordant_z:"${discordant_z}",back_distance:"${back_distance}",weight:"${weight}",min_mapping_threshold:"${min_mapping_threshold}" \
-sr id:sample,bam_file:/usr/local/bin/sample.split.sorted.bam,back_distance:"${back_distance}",weight:"${weight}",min_mapping_threshold:"${min_mapping_threshold}" \
> "${lumpy_vcf}"
